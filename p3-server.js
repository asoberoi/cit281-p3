...
fastify.get("/coin", (request, reply) => {
  const { denom, count = 0 } = request.query;
  const coinValue = coinCount({ denom, count });
  reply
    .code(200)
    .header('Content-Type', 'text/html')
    .send(`<h2>Value of ${count} ${denom}(s) is ${coinValue}</h2><br /><a href="/">Home</a>`);
});
...
fastify.get("/coins", (request, reply) => {
  const coinValueOption = (option) => {
    switch (option) {
      case '1':
        result = coinCount({ denom: 5, count: 3 }, { denom: 10, count: 2 });  // option = 1
        break;
      case '2':
        coins = [{ denom: 25, count: 2 }, { denom: 1, count: 7 }];
        result = coinCount(...coins);    // option = 2
        break;
      case '3':
        coins = [{ denom: 25, count: 2 }, { denom: 1, count: 7 }];
